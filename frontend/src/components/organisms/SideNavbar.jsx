      ...
      >
        ...
        <button
          ...
          className={`hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 z-40
            h-6 w-6 items-center justify-center rounded-full border transition-colors ${darkMode
        ...
        <div className="flex-1 py-6 overflow-hidden">
          {!isSidebarCollapsed && (
            <div className={`px-6 mb-4 text-xs font-bold uppercase tracking-widest ${darkMode ? 'text-slate-500' : 'text-slate-400'
              }`}>
              Core Modules
            </div>
          )}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setIsSidebarOpen(false)}
                >
                  <span className={`group-hover:scale-110 transition-transform flex-shrink-0 ${isActive
