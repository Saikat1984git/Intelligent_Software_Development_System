...
const matchesConditions = (inc, conditions) => {
  ...
  const results = active.map(c => applyOperator(inc[c.field], c.operator, c.value));
...
const BugSupport = () => {
  ...
  const { leftPanelWidth, handleMouseDown, containerRef } = useResizablePanel(58, 35, 75);
  ...
  const filteredIncidents = useMemo(() => {
    return MOCK_INCIDENTS.filter(inc => {
      ...
      if (search.trim()) {
        ...
        if (searchField === 'all') {
          const haystack = [
            inc.id, inc.affected_user, inc.config_item, inc.category,
            inc.priority, inc.state, inc.assignment_group, inc.assigned_to,
            inc.queue, inc.short_desc,
          ].join(' ').toLowerCase();
  ...
  const handleAnalyse = useCallback((incident) => {
    ...
    const steps = [
      { stage: 'understanding', progress: 20, delay: 600, msg: 'Classifying incident type and extracting entities...' },
      { stage: 'understanding', progress: 35, delay: 1200, msg: `Identified: ${incident.category} issue in ${incident.config_item}` },
      { stage: 'retrieval', progress: 50, delay: 2000, msg: 'Searching past incidents and SOP documents...' },
      { stage: 'retrieval', progress: 62, delay: 2800, msg: 'Found 3 similar past incidents.' },
      { stage: 'decision', progress: 74, delay: 3600, msg: 'Running decision engine — matching root cause patterns...' },
      { stage: 'action', progress: 87, delay: 4400, msg: 'Generating recommended actions...' },
      { stage: 'completed', progress: 100, delay: 5200, msg: 'Analysis complete. Guidance ready.' },
    ];
    ...
    steps.forEach(({ stage, progress, delay, msg }) => {
      setTimeout(() => {
        ...
        setLogs(prev => [...prev, {
          id: Date.now() + Math.random(), content: msg,
          timestamp: new Date().toLocaleTimeString(), isError: false,
        }]);
        if (stage === 'completed') {
          ...
          setSelectedIncident(prev => ({
            ...
            ai_analysis: {
              ...
              table: null,
              severity: incident.priority.startsWith('1') ? 'Critical'
                : incident.priority.startsWith('2') ? 'High'
                  : incident.priority.startsWith('3') ? 'Medium' : 'Low',
              ...
              probable_cause: 'Based on similar past incidents and application logs, this appears to be a configuration or data validation issue requiring manual review.',
              recommended_actions: [
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden
      ...
      {/* Header */}
      <header className="shrink-0 h-14 bg-white dark:bg-slate-900
      ...
      <div className="flex-1 flex min-h-0" ref={containerRef}>
        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0
          bg-slate-50 dark:bg-slate-950" style={{ minWidth: '300px' }}>

          <div className="shrink-0 h-10 bg-white dark:bg-slate-900
